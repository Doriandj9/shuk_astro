...
export const sendShareWhatsApp: SharedWhatsAppFn = (text,idPost) => {
    try {
        ...
        url.searchParams.append('text', text + encodeURI(app.host + routesApi.public.view_post.path.replace('{id}', String(idPost)) + '?op=view'));
...
export const sendShareFacebook: ShareFacebookFn = (idPost) => {
    try {
        ...
        url.searchParams.append('u', encodeURI(app.host + routesApi.public.view_post.path.replace('{id}', String(idPost))));
